...
car.Izz          = 100 +10;                      % moment of inertia [kg m^2]
...
car.Sus_t = 0.3;                          % suspension time constant for 1st order system
...
optparams.steeredAxle = 'front'; % 'front', 'rear', 'both'
optparams.drivenAxle  = 'both'; % 'front', 'rear', 'both'
...
optparams.alpha_sat   = 15/180*pi; % Maximum allowed tire slip angle
...
%%%%%%%%%%%%%%%%%%% Track parameters %%%%%%%%%%%%%%%%
track.name = ('doubleTurn'); % 'FSCZ2023' 'doubleTurn', 'doubleTurn2', 'experimental', 'experimental2', 'eight', 'sine', 'blackwood', 'simoncelli', 'accel'
...
if track.name == "accel"
    ds_min = 0.1; ds_max = 0.5;
    s = 0;

    while s(end) < s_traj(end)
        while s(end) < 2
            s(end+1) = s(end) + 0.005;
        end
        Ck = abs(interp1(s_traj, C_traj, s(end)));
...
s_opt = pokus_s_opt(s_traj, C_traj);
...
if car.tracks == 2
    U = opti.variable(7, optparams.N); % controls
    U_delta_f  = U(1, :);
    U_delta_r  = U(2, :);
    U_Ffl      = U(3, :);
    U_Ffr      = U(4, :);
...
switch lower(optparams.method)
    case 'euler'
        ...
        for k=1:optparams.N % loop over control intervals
            x_next = Z(:,k) + (s_opt(k+1)-s_opt(k))*f(s_opt(k), Z(:,k), U(:,k));
    ...
    case 'rk4'
        ...
        for k=1:optparams.N % loop over control intervals
            ...
            k1 = f(s_opt(k),         Z(:,k),         U(:,k));
            k2 = f(s_opt(k) + dsk/2, Z(:,k)+dsk/2*k1, U(:,k));
            k3 = f(s_opt(k) + dsk/2, Z(:,k)+dsk/2*k2, U(:,k));
            k4 = f(s_opt(k) + dsk,   Z(:,k)+dsk*k3,   U(:,k));
...
if car.tracks == 2
    ...
    opti.subject_to(U_Ffr <= optparams.Ff_max/optparams.F_scale);
    opti.subject_to(U_Frl <= optparams.Ff_max/optparams.F_scale);
    opti.subject_to(U_Frr <= optparams.Ff_max/optparams.F_scale);


   % opti.subject_to(U_Ffl >= optparams.Ff_min/optparams.F_scale);
   % opti.subject_to(U_Ffr >= optparams.Ff_min/optparams.F_scale);
   % opti.subject_to(U_Frl >= optparams.Ff_min/optparams.F_scale);
   % opti.subject_to(U_Frr >= optparams.Ff_min/optparams.F_scale);
...
if car.tracks == 2
    ...
    for i = 1:1:length(car.trqSpeedCharSpeed)-1
       % car.trqSpeedCharTrq
   ...
   % opti.subject_to( positivePower <= car.PwrMax);
    ...
    diffabs = @(x) x.*tanh(x.*0.005);
    optparams.PowerBrake_scale = 1;
    car.losses = 0.1;

    Power_fl = velocityWheel_flx.*U_Ffl.*optparams.F_scale;
    Power_fr = velocityWheel_frx.*U_Ffr.*optparams.F_scale;
    Power_rl = velocityWheel_rlx.*U_Frl.*optparams.F_scale;
    Power_rr = velocityWheel_rrx.*U_Frr.*optparams.F_scale;
    ...
    Loss_fl = diffabs(Power_fl - PowerBrake_fl) * car.losses;
    Loss_fr = diffabs(Power_fr - PowerBrake_fr) * car.losses;
    Loss_rl = diffabs(Power_rl - PowerBrake_rl) * car.losses;
    Loss_rr = diffabs(Power_rr - PowerBrake_rr) * car.losses;

    PowerAcp_fl = Power_fl + Loss_fl - PowerBrake .* optparams.PowerBrake_scale./4;
    PowerAcp_fr = Power_fr + Loss_fr - PowerBrake .* optparams.PowerBrake_scale./4;
    PowerAcp_rl = Power_rl + Loss_rl - PowerBrake .* optparams.PowerBrake_scale./4;
    PowerAcp_rr = Power_rr + Loss_rr - PowerBrake .* optparams.PowerBrake_scale./4;
    ...
    EnergyForLap = car.AcpCapacity*60*60*1000/22;
...
if car.tracks == 2
    ...
    opti.set_initial(U_Ffl,     optparams.Ffl_init);
    opti.set_initial(U_Ffr,     optparams.Ffr_init);
    opti.set_initial(U_Frl,     optparams.Frl_init);
    opti.set_initial(U_Frr,     optparams.Frr_init);
...
PowerBrake_opt =  sol.value(PowerBrake)'* optparams.PowerBrake_scale;
%u_opt(:,7) =PowerBrake_opt;
if car.tracks == 2
...
mydata = optdata(expdata.z,expdata.u,car);
...
EnergyUsed = round( sol.value(EnergyUsed));
EnergyForLap;
